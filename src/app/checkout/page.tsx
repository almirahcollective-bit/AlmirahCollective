...
export default function CheckoutPage() {
  ...
  const availableStates = useMemo(() => State.getStatesOfCountry(form.countryIso), [form.countryIso]);
  const availableCities = useMemo(() => City.getCitiesOfState(form.countryIso, form.stateIso), [form.countryIso, form.stateIso]);
  ...
  async function handlePincodeChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Only auto-detect for India
    if (form.countryIso === "IN" && val.length === 6 && /^\d+$/.test(val)) {
      ...
      try {
        ...
        if (data && data[0] && data[0].Status === "Success") {
          ...
          // Find matching state and city in our library
          const matchedState = availableStates.find(s => s.name.toLowerCase().includes(postOffice.State.toLowerCase()) || postOffice.State.toLowerCase().includes(s.name.toLowerCase()));
          if (matchedState) {
            setForm(prev => ({ ...prev, stateIso: matchedState.isoCode }));
            // We set the state, but cities might take a cycle to update, so we can't safely set city here without a small delay or ref
            setTimeout(() => {
              const newCities = City.getCitiesOfState("IN", matchedState.isoCode);
              const matchedCity = newCities.find(c => c.name.toLowerCase().includes(postOffice.District.toLowerCase()) || postOffice.District.toLowerCase().includes(c.name.toLowerCase()));
              if (matchedCity) {
                setForm(prev => ({ ...prev, city: matchedCity.name }));
              } else {
                 setForm(prev => ({ ...prev, city: postOffice.District }));
              }
            }, 50);
          }
        }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Basic phone validation based on length
    ...
    setError("");
    try {
      const countryName = allCountries.find(c => c.isoCode === form.countryIso)?.name || form.countryIso;
      const stateName = availableStates.find(s => s.isoCode === form.stateIso)?.name || form.stateIso;
          ...
          <form onSubmit={onSubmit} className="mt-10 space-y-5">
            <fieldset className="space-y-4">
              ...
              <div className="flex w-full border border-obsidian/15 bg-transparent focus-within:border-champagne">
                <select 
                  className="bg-transparent px-3 py-3 text-sm outline-none border-r border-obsidian/15 text-obsidian/80 appearance-none cursor-pointer w-28 shrink-0 truncate"
                  value={form.phoneCode}
                  onChange={(e) => setForm({ ...form, phoneCode: e.target.value })}
                >
                  {allCountries.map(c => (
                    <option key={`phone-${c.isoCode}`} value={`+${c.phonecode}`}>
                      {c.flag} +{c.phonecode} ({c.isoCode})
                    </option>
                  ))}
                </select>
                <input
            ...
            <fieldset className="space-y-4">
              ...
              <div className="w-full relative border border-obsidian/15 focus-within:border-champagne">
                <select
                  required
                  className="w-full bg-transparent px-4 py-3 text-sm outline-none appearance-none cursor-pointer"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="relative border border-obsidian/15 focus-within:border-champagne">
                  {availableStates.length > 0 ? (
                    <select
                      required
                      className="w-full bg-transparent px-4 py-3 text-sm outline-none appearance-none cursor-pointer truncate"
                      value={form.stateIso}
                      onChange={(e) => setForm({ ...form, stateIso: e.target.value })}
                    >
